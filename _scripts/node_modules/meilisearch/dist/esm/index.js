...
class Index {
   ...
   * @returns Promise containing an EnqueuedTask
   */
  deleteDocuments(params, options) {
   ...
   * @returns Promise containing an EnqueuedTask
   */
  updateDocumentsByFunction(options, taskOptions) {
    return this.#httpRequestsWithTask.post({
   ...
   */
  async getFields(params, extraRequestInit) {
    return await this.httpRequest.post({
...
class MeiliSearch {
  ...
  #validateInitializationShards(remotes, shards) {
    ...
    for (const [shardName, shard] of shardEntries) {
      ...
      const missingRemotes = shard.remotes.filter(
        (remoteName) => remotes[remoteName] === void 0
      );
   ...
   * @experimental
   */
  addRemotesToShard(shardName, remotes) {
    return this.#patchNetwork({
      shards: {
   ...
   * @experimental
   */
  removeRemotesFromShard(shardName, remotes) {
    return this.#patchNetwork({
      shards: {
